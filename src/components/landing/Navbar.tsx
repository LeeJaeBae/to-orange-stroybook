...
const menuItems: MenuItem[] = [
  {
    ...
    subItems: [
      ...
      // { title: '스케줄 관리', url: '/letter/schedule' },
      // { title: '오렌지 나무', url: '/letter/orange-tree' },
      // { title: '타임캡슐', url: '/letter/time-capsule' },
      // { title: '손편지 담기', url: '/letter/handwritten' },
    ],
  },
  {
    title: '회사소개',
...
const myPageSubItems: SubMenuItem[] = [
  ...
  // { title: '소중한 사람', url: '/letter/family' },
  // { title: '발신인 주소', url: '/letter/address' },
  // { title: '결제 내역', url: '/letter/payments' },
  // { title: '설정', url: '/letter/settings' },
];
...
export default function Navbar() {
  ...
  return (
    <nav className={`sticky top-0 z-40 transition-all duration-200 ${
      ...
      {/* 상단 카테고리 헤더 바 - PC 전용 */}
      <div className="hidden nav:block bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex items-center justify-between h-12 nav:h-16">
          {/* 로고 */}
          ...
          {/* PC 메뉴 - 센터 정렬 */}
          <div className="hidden nav:flex items-center justify-center flex-1 space-x-6 lg:space-x-10">
            {menuItems.map((item) => (
              <div
                key={item.title}
                className="relative"
          ...
          <div className="flex items-center gap-2">
            ...
            {isLoggedIn ? (
              <>
                <Link
                  ...
                  className="hidden nav:block bg-orange-500 text-white text-sm font-semibold px-6 py-2.5 rounded-lg hover:bg-orange-600 transition-colors"
                >
                  편지쓰기
                </Link>
                <button
                  onClick={handleLogout}
                  className="hidden nav:block bg-white text-gray-700 text-sm font-semibold px-6 py-2.5 rounded-lg border border-gray-300 hover:border-gray-400 hover:bg-gray-50 transition-colors"
                >
                  로그아웃
                </button>
              </>
            ...
            <button
              ...
              className="nav:hidden w-10 h-10 flex items-center justify-center text-gray-600 hover:text-orange-500 transition-colors"
            >
      ...
      {isMobileMenuOpen && (
        <>
          ...
          />
          <div className="fixed top-0 right-0 bottom-0 w-[280px] bg-white z-50 nav:hidden shadow-xl animate-in slide-in-from-right duration-200">
            ...
            <div className="px-6 pb-6 h-[calc(100%-72px)] overflow-y-auto">
              ...
              <div className="mt-8 space-y-3">
                {isLoggedIn ? (
                  ...
                  <>
                    <Link href="/letter/compose/1" onClick={() => setIsMobileMenuOpen(false)} className="block w-full bg-orange-500 text-white text-center text-base font-semibold py-4 rounded-xl hover:bg-orange-600 transition-colors">편지쓰기</Link>
                    <Link href="/auth" onClick={() => setIsMobileMenuOpen(false)} className="block w-full bg-white text-gray-700 text-center text-base font-semibold py-4 rounded-xl border border-gray-300 hover:border-gray-400 hover:bg-gray-50 transition-colors">로그인</Link>
                  </>
                )}
              </div>
            </div>
